...
export const PureCodeViewer = observer(() => {
  const [editor, setEditor] = useState<
    monacoEditorAPI.IStandaloneCodeEditor | undefined
  >();
  const editorStore = useEditorStore();
  const applicationStore = useApplicationStore();
  const feedbackData = editorStore.feedbackData;
  const content = editorStore.pureCode ?? '';
  ...
  }, [editorStore, applicationStore, editor]);
  ...
  if (editor) {
    ...
    if (editorModel) {
      ...
      if (feedbackData?.output.length) {
        const problems = feedbackData.output;
        problems.forEach((problem) => {
          setErrorMarkers(
            editorModel,
            problem.msg,
            problem.srclocation.startLine,
            problem.srclocation.startColumn,
            problem.srclocation.endLine,
